...
bool runTest(RequestLineTest& t) {
    ...
    try {
        ...
        if (t._isSuccessTest && compareRequestLine(r, t)) {
            ...
            return true;
        } else if (t._isSuccessTest) {
            std::cout << "==== Success test different content "
                << t._name << " ====" << std::endl;
            showRequestLine(r, t);
            return false;
        } else {
            std::cout << "==== Fail test success "
                << t._name << " ====" << std::endl;
            showRequestLine(r, t);
            return false;
        }
        ...
        if (!t._isSuccessTest && compareRequestLine(r, t)) {
            ...
            std::cout << "==== Fail test different content " << t._name
                << " ====" << std::endl;
            showRequestLine(r, t);
        } else {
            std::cout << "==== Success test failed " << t._name
                << " ====" << std::endl;
            showRequestLine(r, t);
        }
    }
...
void makeHttpVersionTests(TestVector& t) {
    ...
    r._name = "空のHTTPバージョン";
    r._request = "GET / \r\nHost: sample\r\n\r\n";
    r._httpStatus = http::BAD_REQUEST;  // OK
    r._isSuccessTest = false;
    r._exceptRequest.method = "GET";
    r._exceptRequest.path = "/";
    r._exceptRequest.version = "";
...
void makePercentEncodingTests(TestVector& t) {
    ...
    r._httpStatus = http::OK;  // will Not Found
    r._isSuccessTest = true;
    r._exceptRequest.method = "GET";
